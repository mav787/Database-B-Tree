...
public abstract class Node{
	...
	public int findPtrIndex (int val){
		// Binary Search
		// find the last index that keys[index] <= val, or 0
	...
	public void delete (int i){
		this.deleteSimple(i);
		// still valid
		if(this.getParent() == null || this.getLast() >= this.minkeys())
			return;
		// combine
		if(this.siblings(next) && this.combinable(next))
			this.combine();
		else if(this.siblings(prev) && this.combinable(prev))
			this.getPrev().combine();
		// redistribute
		else if(this.siblings(next))
			this.redistribute();
		else if(this.siblings(prev))
